Given("there are grades in the gradebook") do #JLS
...
Given("I sign in as a TA") do
    ...
    fill_in "user_password", with: "123greetingsTA"
    click_on "Log in"
end
...
When("I fill out the form and submit") do
    fill_in "grade_student_id", with: "12346"
...
When("I click the {string} string") do |button_name|
    click_on string
end

Then("the student grades should be sorted in ascending order") do
    @grades = Grade.order('student_grade') 
    @grades.each do |grade|
        expect(page.body).to include(grade.student_name)
    end
end

Then("the student grades should be sorted in descending order") do
